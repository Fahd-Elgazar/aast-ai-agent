...
// Readiness §2: the previous default was a hard-coded LAN IP (192.168.1.7),
// so any start without OLLAMA_BASE_URL silently targeted a stray machine.
// Default to loopback; a remote Ollama must be configured intentionally.
const ollamaBaseUrl = stringFromEnv(
  "OLLAMA_BASE_URL",
  "http://127.0.0.1:11434"
).replace(/\/$/, "");

if (!/^https?:\/\/(127\.0\.0\.1|localhost|host\.docker\.internal)(:|\/|$)/i.test(ollamaBaseUrl)) {
  console.warn(
    `[LLM_CONFIG] OLLAMA_BASE_URL points at a non-local host: ${ollamaBaseUrl}. ` +
    "If this is not an intentional remote Ollama deployment, LLM and embedding " +
    "calls will fail or hit the wrong machine."
  );
}
