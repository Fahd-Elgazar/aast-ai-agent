...
class GemmaRequestLimiter {
  ...
  acquire(metadata = {}) {
    ...
    if (this.queue.length >= this.config.gemma.maxQueueDepth) {
      this.syncTelemetry();
      // LG-08: overflow rejections are a primary concurrency failure mode
      // (single active Gemma slot); make them attributable per request.
      console.warn(
        `[GEMMA_QUEUE][${metadata.requestId || "unknown"}] result=OVERFLOW ` +
        `active=${this.active} pending=${this.queue.length} ` +
        `maxActive=${this.config.gemma.maxActiveRequests} maxQueueDepth=${this.config.gemma.maxQueueDepth} ` +
        `route=${metadata.routeType || "unknown"}`
      );
