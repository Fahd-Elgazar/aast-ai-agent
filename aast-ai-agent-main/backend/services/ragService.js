...
class RAGService {
    ...
    // ══════════════════════════════════════════════════════════
    // PUBLIC API — CIRCUIT BREAKER STATUS (RC-07)
    // ══════════════════════════════════════════════════════════

    /**
     * Reports whether the circuit breaker would reject a search() call
     * right now. Consumed by healthProbes so the router's health signal
     * agrees with actual request-path availability.
     *
     * `blocking` is true only while OPEN *and* within the cooldown window.
     * Once the cooldown elapses, the next request transitions the breaker
     * to HALF_OPEN, so we report non-blocking to let traffic resume and
     * drive recovery.
     */
    getCircuitBreakerStatus() {
        ...
        const blocking =
            ...
            (Date.now() - cb.last_failure) < CONFIG.CB_COOLDOWN_MS;
     ...
     * @param {Object} [options] - { traceId } for cross-service correlation.
     *   NOTE: options.topK is intentionally NOT honored — retrieval depth is
     *   governed centrally by CONFIG.TOP_K / TOP_K_DEEP so pass semantics
     *   stay uniform across callers.
     * @returns {Promise<Object>} Canonical result envelope
     */
    async search(query, options = {}) {
        const traceContext = { traceId: options.traceId || null };
        ...
        const pass1 = await this._callRetriever(expandedQuery, CONFIG.TOP_K, queryCategory, traceContext);
        ...
        const pass2 = await this._callRetriever(simplifiedQuery, CONFIG.TOP_K_DEEP, queryCategory, traceContext);
        ...
        const pass3 = await this._callAnswerEngine(query, traceContext);
     ...
     */
    async requestWithRetry(url, payload, context = {}) {
        logger.debug('SEARCH', `POST ${url}`, { payload, trace_id: context.traceId || null });
        ...
        try {
            ...
            const response = await httpClient.post(url, payload, {
                ...
                // Cross-service correlation (LG: trace-id gap)
     ...
     * @private
     */
    async _callRetriever(query, topK, category, context = {}) {
     ...
     * @private
     */
    async _callAnswerEngine(query, context = {}) {
        const result = await this._callWithEndpointFallback(
            CONFIG.ANSWER_URL,
            CONFIG.ANSWER_PATH,
     ...
     * @private
     */
    async _callWithEndpointFallback(baseUrl, primaryPath, altPath, payload, serviceKey, context = {}) {
        const primaryUrl = `${baseUrl}${primaryPath}`;
        const altUrl = `${baseUrl}${altPath}`;

        try {
            const data = await this.requestWithRetry(primaryUrl, payload, context);
            ...
            if (primaryErr._ragErrorType === 'ENDPOINT_NOT_FOUND') {
                ...
                try {
                    const data = await this.requestWithRetry(altUrl, payload, context);
