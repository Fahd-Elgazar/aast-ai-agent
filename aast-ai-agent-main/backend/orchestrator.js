...
import { checkSubsystemHealth, timeoutWrapper, getHealthCacheAge } from "./services/healthProbes.js";
...
// Node's process state is undefined after an uncaughtException; log then exit
// so the process supervisor (docker/launcher) restarts a clean instance instead
// of continuing to serve requests from a corrupted process.
...
function getGoldenCachedPayload(goldenMatch) {
  ...
  // LG-06: golden cache reads are a routing-relevant event.
  console.log(`[GOLDEN_CACHE] id=${goldenMatch.id} hit=true ageMs=${Date.now() - entry.time}`);
  return cloneJson(entry.payload);
}

/**
 * RC-08 guard: a degraded or static-fallback payload must NEVER be cached.
 * Previously, a golden query asked while a subsystem was momentarily down
 * (cold embed model, health flap) froze its degraded answer for the full
 * 10-minute TTL — the single stickiest source of "sometimes perfect,
 * sometimes bad, and it won't change".
 */
function isGoldenPayloadCacheable(payload) {
  if (!payload) return false;
  if (payload.degraded === true) return false;
  // buildGoldenFallbackPayload() marks every static fallback with a reason.
  if (payload.explainability?.fallback_reason) return false;
  return true;
}
...
function setGoldenCachedPayload(goldenMatch, payload) {
  ...
  if (!isGoldenPayloadCacheable(payload)) {
    incrementMetric("golden_cache_degraded_write_skipped");
    console.log(`[GOLDEN_CACHE_WRITE] id=${goldenMatch.id} skipped=true reason=degraded_payload`);
    return;
  }
  ...
  console.log(`[GOLDEN_CACHE_WRITE] id=${goldenMatch.id} skipped=false route=${payload.route_used || payload.route || "unknown"}`);
...
// RC-15 — PER-CONVERSATION SERIALIZATION
// getConversation() returns the live object from the in-memory Map and the
// handler mutates it (lastRoute, lastCurriculumCourse, messages). Two
// concurrent requests on the same cid interleaved those mutations. Serialize
// turns per conversation; different conversations still run fully parallel.
const conversationLocks = new Map();

function withConversationLock(cid, fn) {
  const previous = conversationLocks.get(cid) || Promise.resolve();
  const run = previous.then(fn);
  const tail = run.catch(() => {}).then(() => {
    if (conversationLocks.get(cid) === tail) conversationLocks.delete(cid);
  });
  conversationLocks.set(cid, tail);
  return run;
}

// Sentinel distinguishing "KG retrieval abandoned at route timeout" from a
// legitimate null result. Module-scoped so identity comparison is reliable.
const KG_RETRIEVAL_ABANDONED = Object.freeze({ abandoned: true });
...
app.post("/api/chatbot/query", (req, res) => {
  // Serialize turns within one conversation (RC-15); new/absent cids get a
  // fresh conversation each and need no lock.
  const lockKey = normalizeConversationId(req.body?.cid);
  if (lockKey) {
    return withConversationLock(lockKey, () => handleChatbotQuery(req, res));
  }
  return handleChatbotQuery(req, res);
});

async function handleChatbotQuery(req, res) {
  // F2.1 (RC-15): UUID trace id — Date.now() collided within the same
  // millisecond and could not correlate a request across services.
  ...
  res.json = function (body) {
    const sendBody = (finalBody) => {
      ...
      // LG "why did this differ": one consolidated line recording every
      // non-stationary input that could have changed the outcome, so any
      // "same question, different answer" incident is a single grep away.
      const explainTrace = finalBody?.metadata?.trace || {};
      console.log(
        `[REQ_EXPLAIN][${requestId}] cid=${finalBody?.cid || "unknown"} ` +
        `route=${explainTrace.route || finalBody?.route || "unknown"} ` +
        `tier=${explainTrace.response_tier || "unknown"} ` +
        `conf=${finalBody?.confidence ?? "n/a"} ` +
        `degraded=[${(explainTrace.degraded_services || []).join(",")}] ` +
        `health=${JSON.stringify(explainTrace.subsystem_health || {})} ` +
        `latencyMs=${duration}`
      );
  ...
  try {
    ...
    // LG-01: the exact snapshot that gates this request's routing. Diffing this
    // line across two identical asks answers "why did the route change".
    const healthCacheAgeMs = getHealthCacheAge();
    console.log(
      `[HEALTH_SNAPSHOT][${requestId}] cid=${conversationId} ` +
      `kg=${healthStatus.kg} rag=${healthStatus.rag} decision=${healthStatus.decision} ` +
      `career=${healthStatus.career} faq=${healthStatus.faq} llm=${healthStatus.llm} ` +
      `cacheAgeMs=${Number.isFinite(healthCacheAgeMs) ? healthCacheAgeMs : "never_probed"} ` +
      `golden=${goldenMatch?.id || "none"}`
    );

    // RC-04: the historical-route boost previously applied to EVERY query,
    // letting the previous turn's route flip routing for unrelated questions
    // near an ambiguity margin. Continuity bias is only semantically valid
    // when the current query actually references the previous turn.
    const routeContinuityEligible = Boolean(followUpResolution?.detected);
    const analysisPayload = brainRouter.analyzeQuery(
      { query, intent: intentKeyword, normalization: normalizationTrace },
      intentKeyword,
      { lastRoute: routeContinuityEligible ? (convo.lastRoute || null) : null }
    );
    ...
    // LG-02: decision inputs vs output in one line — shows when an override
    // or historical-route bias flipped the route relative to the raw signals.
    const topSignals = Object.entries(analysisPayload.signals || {})
      .sort((a, b) => b[1] - a[1])
      .slice(0, 2)
      .map(([name, value]) => `${name}=${value}`);
    console.log(
      `[ROUTE_DECISION][${requestId}] intent=${intentKeyword} ` +
      `lastRouteApplied=${routeContinuityEligible ? (convo.lastRoute || "none") : "not_eligible"} ` +
      `initial=${initialRoute} final=${route} conf=${routingDecision.confidence} ` +
      `overrides=[${routeOverrideReasons.join(",")}] topSignals=[${topSignals.join(",")}]`
    );
    ...
    const runNeo4jRetrieval = async ({
      ...
      // Release the semaphore when the ACTUAL retrieval settles (not when the
      // race below settles) so an abandoned-but-running query still occupies
      // its Neo4j slot until it truly finishes.
      ...
      retrievalPromise.then(releaseOnce, releaseOnce);

      // F1.4 (RC-03/RC-06): the KG path previously had NO orchestrator-level
      // timeout — kgTimeoutMs was computed and logged but never enforced, so a
      // cold embedding model stalled requests for up to 60s (30s embed timeout
      // x2 attempts) or escalated to the fatal catch on embed failure. Bound
      // it and degrade cleanly like every other route. timeoutWrapper also
      // converts retrieval ERRORS to the sentinel, so an embed failure now
      // degrades to the KG_TIMEOUT path instead of a fatal fallback.
      try {
        const raced = await timeoutWrapper(
          retrievalPromise,
          kgTimeoutMs,
          KG_RETRIEVAL_ABANDONED,
          `kg_retrieval_${resolvedIntent}`
        );

        if (raced === KG_RETRIEVAL_ABANDONED) {
          incrementMetric("kg_route_timeout_abandoned");
          logger.warn("KG retrieval abandoned at route timeout; degrading", {
            requestId,
            intent: resolvedIntent,
            timeout_ms: kgTimeoutMs
          });
          // Diagnose late completions without resurrecting them (RC-08 hygiene:
          // the response pipeline must not incorporate results after abandonment).
          retrievalPromise.then(
            () => console.warn(`[KG_LATE_COMPLETION][${requestId}] retrieval finished after abandonment intent=${resolvedIntent}`),
            (err) => console.warn(`[KG_LATE_FAILURE][${requestId}] intent=${resolvedIntent} error="${err?.message}"`)
          );
          return null;
        }
    ...
    // KG timeout is now ENFORCED (F1.4). Non-golden default raised 5000 -> 12000:
    // a cold nomic-embed-text load can take several seconds, and 12s bounds the
    // worst-case stall while still giving first-touch embeddings a chance.
    const kgTimeoutMs = goldenMatch ? Math.min(deterministicTimeoutMs, 2800) : timeoutFromEnv(["KG_ROUTE_TIMEOUT_MS"], 12000);
      ...
      for (const intentPart of multiIntentPlan.intents) {
        ...
        if (intentPart.retrieval === "RAG") {
          ...
          const intentRagRaw = await timeoutWrapper(
            ragService.search(scopedQuery, { topK: 3, traceId: requestId }),
            ragTimeoutMs,
            null,
            "rag_multi_intent"
          ).finally(() => ragSemaphore.release());
...
Fallback Calls: 0`);
        ...
        await ragSemaphore.acquire();
        const ragRes = await timeoutWrapper(ragService.search(query, { topK: 5, traceId: requestId }), ragTimeoutMs, null, "rag_route").finally(() => ragSemaphore.release());
...
Sources: ${rawResults.rag.results.length}`);
        ...
        const [kgOutcome, ragOutcome] = await Promise.allSettled([
          ...
          ragSemaphore.acquire().then(() => timeoutWrapper(ragService.search(query, { topK: 5, traceId: requestId }), hybridRouteTimeoutMs, null, "rag_hybrid").finally(() => ragSemaphore.release()))
        ]);
        ...
        if (comparisonRequested) {
          ...
          const decisionResult = await timeoutWrapper(
            ...
            decisionTimeoutMs, null, "decision_engine"
          );
        ...
        if ((!memory || Object.keys(memory).length === 0) && !goldenMatch && !inferredAiCareer) {
          ...
          const careerMemory = (!memory || Object.keys(memory).length === 0)
            ...
            : memory;
          const roadmap = await timeoutWrapper(Promise.resolve(buildCareerRoadmap(null, null, careerMemory)), goldenMatch ? 1600 : 5000, null, "career_engine");
          if (roadmap) {
            rawResults.career = [{
...
Respond briefly, professionally, and conversationally as the AAST Advisor.`;
        ...
        if (runtimeMode.singleGemmaGenerationMode) {
          ...
          llmText = await timeoutWrapper(
            generateStableResponse({
              ...
              options: {
                // RC-12: greedy under deterministic synthesis (default) so the
                // fallback answer for an identical prompt is stable run-to-run.
                temperature: runtimeMode.deterministicSynthesis ? 0 : 0.08,
                top_p: runtimeMode.deterministicSynthesis ? 0.1 : 0.72,
                repeat_penalty: 1.18,
                num_predict: 220
              }
            }),
            fallbackDeadlineMs, null, "llm_fallback_generation"
      ...
      if (goldenMatch) {
        // RC-08: a synthesis result produced while subsystems were degraded is
        // partial evidence — do not freeze it for the cache TTL. The central
        // guard also rejects payloads self-marked degraded; this additionally
        // covers partial hybrid failures that only degraded_services records.
        if (degraded_services.length === 0) {
          setGoldenCachedPayload(goldenMatch, fusionPayload);
        } else {
          incrementMetric("golden_cache_degraded_write_skipped");
          console.log(`[GOLDEN_CACHE_WRITE] id=${goldenMatch.id} skipped=true reason=degraded_services:[${degraded_services.join(",")}]`);
        }
        writeRoutingAudit({
          requestId,
          cid: conversationId,
