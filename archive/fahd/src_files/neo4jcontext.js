...
const MAX_FACTS_BY_INTENT = {
  ...
  PROGRAM: Number.parseInt(process.env.KG_PROGRAM_MAX_FACTS || "24", 10),
  COMPARE: Number.parseInt(process.env.KG_PROGRAM_MAX_FACTS || "24", 10),
...
const PROTECTED_QUERY_INTENTS = new Set(["TEACHING", "PREREQUISITE", "ADMIN", "PERSON"]);
...
function detectIntent(query, requestedIntent = "ALL") {
  ...
  if (/\b(require|requires|need|needs)\b/.test(normalizedQuery) && hasCourseRequirementContext(normalizedQuery)) {
    return "PREREQUISITE";
  }
...
function buildExactTeachingFallbackCypher(resultLimit) {
  return `
    MATCH (staffNode)-[teachesRel:TEACHES]->(courseNode:Course)
    WHERE coalesce(staffNode.name, "") <> ""
      AND (
        (size($contentPhraseKeywords) > 0 AND ${exactEntityPredicate("courseNode", "$contentPhraseKeywords")})
        OR (size($contentAcronymKeywords) > 0 AND ${exactEntityPredicate("courseNode", "$contentAcronymKeywords")})
        OR (
          size($contentPhraseKeywords) = 0
          AND size($contentAcronymKeywords) = 0
          AND ${keywordContainsPredicate("courseNode", "staffNode", "$contentKeywords")}
        )
      )

    WITH courseNode, staffNode, teachesRel,
...
function buildExactPrerequisiteFallbackCypher(resultLimit, reverse = false) {
  const targetPredicate = reverse
    ? keywordContainsPredicate("prereqNode", "courseNode", "$contentKeywords")
    : keywordContainsPredicate("courseNode", "prereqNode", "$contentKeywords");

  return `
    MATCH (courseNode:Course)-[prereqRel:HAS_PREREQUISITE]->(prereqNode:Course)
    WHERE coalesce(courseNode.name, "") <> ""
      AND coalesce(prereqNode.name, "") <> ""
      AND ${targetPredicate}

    WITH courseNode, prereqNode, prereqRel,
      1.0 AS semanticScore,
      true AS nodeKeywordMatch,
      true AS relatedKeywordMatch,
      3.0 AS boostedScore,
...
function buildExactProgramCourseFallbackCypher(resultLimit) {
  return `
    MATCH (programNode)-[courseRel:HAS_COURSE]->(courseNode:Course)
    WHERE coalesce(courseNode.name, "") <> ""
      AND ANY(programLabel IN labels(programNode) WHERE programLabel IN ["Program", "Specialization", "Track"])
      AND ${keywordContainsPredicate("programNode", "courseNode", "$contentKeywords")}

    WITH programNode, courseNode, courseRel,
      1.0 AS semanticScore,
      true AS nodeKeywordMatch,
      true AS relatedKeywordMatch,
      3.0 AS boostedScore,
    ...
    ${returnProjection("programNode", "courseNode", "courseRel", "relatedName ASC")}
    LIMIT ${resultLimit}
  `;
}

function isReversePrerequisiteQuery(query) {
  const normalized = normalizeText(query);
  return /\bcourses?\b.*\b(require|requires|required|need|needs)\b/.test(normalized)
    || /\bwhat\b.*\bcourses?\b.*\b(linear algebra|fundamentals|machine learning|deep learning|prerequisite)\b/.test(normalized);
}

async function retrieveExactKeywordFallback(session, detectedIntent, retrievalQuery, keywordParams, resultLimit, requestId) {
  if (!keywordParams?.contentKeywords?.length) return [];

  let cypher = null;
  if (detectedIntent === "TEACHING") {
    cypher = buildExactTeachingFallbackCypher(resultLimit);
  } else if (detectedIntent === "PREREQUISITE") {
    cypher = buildExactPrerequisiteFallbackCypher(resultLimit, isReversePrerequisiteQuery(retrievalQuery));
  } else if (detectedIntent === "PROGRAM") {
    cypher = buildExactProgramCourseFallbackCypher(resultLimit);
  }

  if (!cypher) return [];

  const records = await runRetrieval(session, cypher, keywordParams);
  if (records.length > 0) {
    incrementMetric("knowledge_graph.exact_keyword_fallback_hit");
    logger.info("Neo4j exact keyword fallback hit", {
      requestId,
      intent: detectedIntent,
      recordsCount: records.length,
      contentKeywords: keywordParams.contentKeywords
    });
  }
...
export async function fetchNeo4jContext(query, intent = "ALL", limit = DEFAULT_CONTEXT_LIMIT, requestId = "none", lastMessages = "", options = {}) {
  ...
  const requestedOntologyIntent = ONTOLOGY_INTENTS.has(normalizeIntentKeyword(intent))
    ...
    : null;
  const queryDetectedIntent = detectIntent(retrievalQuery, intent);
  const explicitQueryIntent = PROTECTED_QUERY_INTENTS.has(queryDetectedIntent) ? queryDetectedIntent : null;
  const detectedIntent = explicitQueryIntent || requestedOntologyIntent || exactOntologyEntity?.intent || queryDetectedIntent;
  ...
  const resultLimit = Math.max(effectiveLimit * 3, MAX_FACTS_BY_INTENT[detectedIntent] || 8, 8);
  ...
  try {
    ...
    let records = await retrieveExactKeywordFallback(
      session,
      detectedIntent,
      retrievalQuery,
      keywordParams,
    ...
    let usedThreshold = records.length > 0 ? 1.0 : null;
    let usedIndexName = records.length > 0 ? "exact_keyword_cypher" : null;
...
${retrievalQuery}`;
      ...
      ({ records, usedThreshold, usedIndexName } = await retrieveWithThresholds(session, primaryPlan, baseParams));
