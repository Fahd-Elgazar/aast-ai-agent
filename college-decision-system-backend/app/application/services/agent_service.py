...
# Tool/instruction definitions are identical for every AgentService instance,
# so they're module-level constants rather than rebuilt per __init__ call.
...
@lru_cache(maxsize=1)
def _get_shared_gemini_model():
    # The Gemini client is stateless and safe to share across requests; building
    # a new one per chat message added avoidable per-request latency and meant
    # every message repeated the same client-init failure mode independently.
