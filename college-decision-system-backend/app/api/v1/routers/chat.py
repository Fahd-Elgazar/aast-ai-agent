...
def get_agent_service() -> Iterator[AgentService]:
    db = SessionLocal()
    try:
        ...
        # The Gemini client itself is a shared singleton (see _get_shared_gemini_model
        # in agent_service.py); only the per-request DB-bound repos are built fresh here.
        yield AgentService(recommend_use_case=use_case, chat_repo=chat_repo)
    finally:
        db.close()
