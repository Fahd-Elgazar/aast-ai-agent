...
async def voice_to_decision(
    ...
    try:
        ...
        try:
            ...
            # Covers non-ValueError failures from the extraction backend (e.g. a
            # Gemini timeout/auth/quota error), which previously propagated as an
            # unhandled 500 with no client-facing explanation.
            raise HTTPException(status_code=502, detail=f"Profile extraction service failed: {e}")
